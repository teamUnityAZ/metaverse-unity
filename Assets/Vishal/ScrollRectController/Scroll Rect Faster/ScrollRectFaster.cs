...
namespace UnityEngine.UI
{
    ...
    public class ScrollRectFaster : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected virtual void LateUpdate()
        {
            if (!m_Content)
                return;
            //Debug.LogError("ScrollRectFaster lateupdate");
                ...
                for (int axis = 0; axis < 2; axis++)
                {
                    ...
                    else if (m_Inertia)
                    {
                        ...
                        if (Mathf.Abs(m_Velocity[axis]) < 1)
                            m_Velocity[axis] = 0;
                        position[axis] += m_Velocity[axis] * deltaTime * 3f;
                    }
        ...
        public float verticalEndPos//rik
        {
            get
            {
                return content.sizeDelta.y - content.anchoredPosition.y;
            }
            set
            {
                SetNormalizedPosition(value, 1);
